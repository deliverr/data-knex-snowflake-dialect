...
module.exports = function (knex) {
  ...
  describe("knex.migrate", function () {
    ...
    if (knex.client.driverName === "sqlite3") {
      it("should not fail rename-and-drop-column with multiline sql from legacy db", async () => {
        const knexConfig = _.extend({}, config.sqlite3, {
          ...
          migrations: {
            directory: "test/integration/migrate/rename-and-drop-column-with-multiline-sql-from-legacy-db",
          },
        });
    ...
    if (knex.client.driverName === "pg") {
      it("should not fail drop-and-recreate-column operation when using async/await and schema", () => {
        return knex.migrate
          .latest({
            directory: "test/integration/migrate/async-await-drop-and-recreate-with-schema",
          })
          .then(() => {
            return knex.migrate.rollback({
              directory: "test/integration/migrate/async-await-drop-and-recreate-with-schema",
    ...
    describe("knex.migrate.latest", function () {
      ...
      it("should throw error if the migrations are already running", function () {
        return knex("knex_migrations_lock")
          .update({ is_locked: 1 })
          .then(function () {
            return knex.migrate.latest({ directory: "test/integration/migrate/test" }).then(function () {
        ...
        return migrator
          ...
          .catch(function (error) {
            ...
            expect(migrator)
              ...
              .to.have.property("fileName", "20150109002832_invalid_migration.js");
          })
          .then(function (data) {
            // Clean up lock for other tests
    ...
    describe("knex.migrate.latest - multiple directories", () => {
      before(() => {
        return knex.migrate.latest({
          directory: ["test/integration/migrate/test", "test/integration/migrate/test2"],
        });
      });
    ...
    describe("knex.migrate.rollback", function () {
      it("should delete the most recent batch from the migration log", function () {
        return knex.migrate.rollback({ directory: "test/integration/migrate/test" }).then(([batchNo, log]) => {
    ...
    describe("knex.migrate.up", () => {
      afterEach(() => {
        return knex.migrate.rollback({ directory: "test/integration/migrate/test" }, true);
      });
    ...
    describe("knex.migrate.down", () => {
      ...
      it("should not error if all migrations have already been undone", function () {
        return knex.migrate.rollback({ directory: ["test/integration/migrate/test"] }, true).then(() => {
          return knex.migrate
            .down({
              directory: ["test/integration/migrate/test"],
            })
  ...
  describe("knex.migrate.latest in parallel", function () {
    ...
    it("is not able to run two migrations in parallel when transactions are disabled", function () {
      ...
      return Promise.all(migrations.map((migration) => migration.then((res) => res && res.name))).then(function (res) {
        // One should fail:
  ...
  describe("knex.migrate.latest with custom config parameter for table name", function () {
    before(function () {
      return knex.withUserParams({ customTableName: "migration_test_2_1a" }).migrate.latest({
        directory: "test/integration/migrate/test",
      });
    });
    ...
    after(function () {
      return knex.withUserParams({ customTableName: "migration_test_2_1a" }).migrate.rollback({
        directory: "test/integration/migrate/test",
      });
    });
  });
  ...
  describe("knex.migrate.latest with disableValidateMigrationList", function () {
    it("should not fail if there is a missing migration", () => {
      return knex.migrate
        ...
        .then(() => {
          return knex.migrate.latest({
            directory: "test/integration/migrate/test_with_missing_first_migration",
